...
public class CollectionsQueryTransformerImplTest extends AbstractMockitoTest
{
	...
	public Iterator<Object[]> transformableQueryProvider()
	{
		...
		{
			Query query = new QueryImpl<>().in(EXPAND_ATTRIBUTE, singletonList("disease")).and().search("test");
			Query expected = new QueryImpl<>().in(EXPAND_ATTRIBUTE, expandedDiseaseEntities).and().search("test");
			dataList.add(new Object[] { query, expected });
		}
